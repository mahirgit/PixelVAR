...
The remaining gaps are clear: user study and 64x64 experiments. The smaller
proposal-leftover ablations are now runnable: 8-color palette, 32-color palette,
and a 4-scale hierarchy. They still need GPU runs before they can be promoted
into the metric tables. MDIGAN has been reviewed and should be cited as related
work rather than used as a direct numeric baseline, because it solves conditional
paired-pose imputation instead of unconditional sprite generation.
