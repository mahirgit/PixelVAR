...
def test_deterministic_pyramid_supports_vq_latent_maps():
    tokenizer = DeterministicPyramidTokenizer([1, 2, 4, 8])
    index_map = torch.arange(8 * 8, dtype=torch.long).reshape(8, 8) % 256

    maps = tokenizer.encode(index_map)
    sequence = tokenizer.to_sequence(maps)
    restored = tokenizer.from_sequence(sequence)

    assert [tuple(m.shape) for m in maps] == [(1, 1), (2, 2), (4, 4), (8, 8)]
    assert sequence.shape == (85,)
    assert torch.equal(restored[-1], index_map)
