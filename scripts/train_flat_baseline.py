#!/usr/bin/env python3
"""Train flat PixelVAR baseline models with Lightning."""
...
from pixelvar.training import LitFlatAR, LitFlatMaskGIT
...
def build_module(config: dict):
    model_kind = str(config.get("model_kind", "flat_ar"))
    optimizer_config = config.get("optimizer", {})
    optimizer_config.setdefault("max_epochs", config.get("trainer", {}).get("max_epochs", 100))
    if model_kind == "flat_ar":
        return LitFlatAR(model_config=config.get("model", {}), optimizer_config=optimizer_config)
    if model_kind == "flat_maskgit":
        return LitFlatMaskGIT(
...
def write_run_report(config: dict, run_name: str, output_dir: Path, checkpoint_callback: ModelCheckpoint) -> None:
    ...
    report = [
        ...
        "",
        "## Notes",
        "- Flat 32x32 palette-token baseline training run.",
        "- Compare via exported PNG folders and the known-metrics evaluator.",
    ]
    (run_dir / "run_report.md").write_text("\n".join(report))


def main() -> None:
    parser = argparse.ArgumentParser(description="Train flat PixelVAR baselines")
